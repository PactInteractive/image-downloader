...
export async function buildCss() {
	await $`bunx @tailwindcss/cli -i ./src/style.css -o ./${config.outputDirectory}/src/style.css`;
}
...
async function generateIcons({ inputSvg, outputDirectory, prefix, sizes }) {
	try {
		...
		for (const size of sizes) {
			const outputPath = join(config.outputDirectory, outputDirectory, `${prefix}${size}.png`);

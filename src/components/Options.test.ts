...
beforeEach(() => {
	...
	($.fn as any).trigger = function (eventName: string, ...args: any[]) {
		if (this.length > 0 && this[0].dispatchEvent) {
			const EventConstructor = eventName === 'click' || eventName === 'dblclick' ? MouseEvent : Event;
			this[0].dispatchEvent(new EventConstructor(eventName, { bubbles: true, cancelable: true }));
			return this;
		}
	...
	HTMLElement.prototype.dispatchEvent = function (event: Event) {
		if (event.type === 'change') {
			return originalDispatch.call(this, new Event('input', { bubbles: true, cancelable: true }));
		}
		...
		describe(option.key, () => {
			option.values.forEach((value) => {
				it(value.toString(), () => {
					...
					expect(localStorage[option.key]).toBe(localStorage[`${option.key}_default`]);
				});
			});
		});
	});
